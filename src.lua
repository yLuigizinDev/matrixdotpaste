...
WatermarkLOLRetard.Name = "WatermarkLOLRetard"
WatermarkLOLRetard.Parent = game.CoreGui
WatermarkLOLRetard.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
WatermarkLOLRetard.ResetOnSpawn = false

WatermarkLOLRetardFrameLOL.Name = "WatermarkLOLRetardFrameLOL"
WatermarkLOLRetardFrameLOL.Parent = WatermarkLOLRetard
WatermarkLOLRetardFrameLOL.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
WatermarkLOLRetardFrameLOL.BorderColor3 = Color3.fromRGB(255, 255, 255)
WatermarkLOLRetardFrameLOL.BorderSizePixel = 2
WatermarkLOLRetardFrameLOL.Position = UDim2.new(0.00752823101, 0, 0.0322180912, 0)
WatermarkLOLRetardFrameLOL.Size = UDim2.new(0, 272, 0, 22)
WatermarkLOLRetardFrameLOL.Visible = false

_1.Name = "1"
_1.Parent = WatermarkLOLRetardFrameLOL
_1.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
_1.BackgroundTransparency = 1.000
_1.Position = UDim2.new(-1.85966492e-05, 0, 0, 0)
_1.Size = UDim2.new(0, 279, 0, 22)
...
local function AULM_fake_script() -- _1.LocalScript 
...
aimbot:Element("Toggle", "aimbot package") 
aimbot:Element("Toggle", "anti godmode (insta kill)") 
...
antiaim:Element("Dropdown", "pitch", {options = {"none", "up", "down", "zero", "180", "random", "huge", "pi"}})
...
client:Element("Toggle", "watermark visible", nil, function(tbl)
	if tbl.Toggle then
		WatermarkLOLRetardFrameLOL.Visible = true
    else
        WatermarkLOLRetardFrameLOL.Visible = false
    end
end)
	...
	if PlayerIsAlive then
        ...
        if values.misc.movement["bunny hop"].Toggle then
    		...
    		if UserInputService:IsKeyDown("Space") then
                ...
                game.CoreGui.BH.BHF.TEXT.TextColor3 = Color3.fromRGB(255,0,5)
            end
        else
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then
			...
			local Pitch = values.rage.angles["pitch"].Dropdown == "none" and CamLook.Y or values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "zero" and 0 or values.rage.angles["pitch"].Dropdown == "huge" and HUGE or values.rage.angles["pitch"].Dropdown == "pi" and pinum or values.rage.angles["pitch"].Dropdown == "random" and math.random(-10, 10)/10 or 2.5
...
mt.__namecall = function(self, ...)
	...
	if method == "FireServer" and self.Name == "HitPart" then
        ...
        if (values.rage.aimbot["aimbot package"].Toggle and RageTarget ~= nil) then 
			coroutine.wrap(function() 
				if Players:GetPlayerFromCharacter(args[1].Parent) or args[1] == RageTarget then 
		...
		if (values.rage.aimbot["anti godmode (insta kill)"].Toggle and RageTarget ~= nil) then 
			coroutine.wrap(function() 
				RageTarget.Parent.Humanoid.Health = 0
			end)() 
		end 
